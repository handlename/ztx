# Summary

[はじめに](introduction.md)

# はじめの一歩

- [インストール](getting-started/installation.md)
- [最初のセッション](getting-started/first-session.md)
- [Zed のセットアップ](getting-started/zed-setup.md)

# ガイド

- [セッション名](guide/session-names.md)
- [ログからファイルを開く](guide/open-files.md)
- [セッションログをエクスポートする](guide/export-log.md)
- [プロジェクトごとに1セッション](guide/one-session-per-project.md)

# リファレンス

- [サブコマンド](reference/subcommands.md)
- [キーバインド](reference/key-bindings.md)
- [設定](reference/configuration.md)
- [環境変数](reference/environment-variables.md)

# トラブルシューティング

- [トラブルシューティング](troubleshooting.md)

# 付録

- [アーキテクチャ](appendix/architecture.md)
- [用語集](appendix/glossary.md)
