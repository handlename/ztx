...
//!
//! Design constraint (see DESIGN.md): ztx does not maintain a full
...
//!   frame is all ztx can — and needs to — capture for hint mode and
//!   export snapshots.
...
pub struct TapShared {
    ...
    /// flood stdin while ztx reads a key.
...
impl Scrollback {
    ...
    pub fn dump(&mut self) -> std::io::Result<String> {
        ...
        if self.dropped > 0 {
            out.push_str(&format!(
                "[ztx: {} earliest lines dropped ({} MiB spill cap)]\n",
                self.dropped,
