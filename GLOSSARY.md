...
| Term | Definition |
|------|------------|
| **Agent CLI** | A terminal-based AI coding agent such as Claude Code (`claude`) or antigravity-cli (`agy`). The process zedic wraps. |
...
| **Wrapper** | The `zedic run` process: a PTY proxy sitting between the parent terminal (Zed) and the agent CLI. |
| **PTY proxy** | The core passthrough: zedic owns a pseudo-terminal, runs the child inside it, and relays bytes both ways while observing them. |
...
| **Session log** | What the session did. Two sources: the CLI's **native transcript** (e.g. Claude Code's session JSONL) and zedic's own **scrollback capture**. |
| **Scrollback** | zedic's ANSI-stripped, line-oriented record of child output on the primary screen. Bounded ring buffer with a temp-file spill. |
...
| **Project socket** | Each session's socket is named by a hash of its project directory (`<hash>.sock`), so `zedic send` finds it in O(1) from the project root. One session per project; a sibling `<hash>.info` records pid + cwd for `sessions`. |
