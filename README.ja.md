...
部分的な重複は「同等」ではありません。Zed の機能が必要性の一部しか満たさない場合は
両方を残し、その違いをドキュメントに記載します。`ztx send` と Zed の
`agent::AddSelectionToThread`（`cmd->`）が現在の例です。
...
ztx setup zed            # Zed のタスクとキーバインドを一度だけ設定する
```
